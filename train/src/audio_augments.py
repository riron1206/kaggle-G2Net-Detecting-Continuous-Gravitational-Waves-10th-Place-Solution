''' 
前回G2net 2位 のくまさんのコード
元ファイル: https://github.com/analokmaus/kaggle-g2net-public/blob/2a85695c1cdd1bdf618ab9df22d3abd188e6a690/transforms.py
...
class GaussianNoiseSNR(AudioTransformPerChannel):
    ...
    def apply(self, y: np.ndarray):
        ...
        if y.ndim == 1:
            white_noise = np.random.randn(len(y))  # for 1ch
        elif y.ndim == 2:
            white_noise = np.random.randn(y.shape[0], y.shape[1])  # for 2ch
...
class GaussianNoiseSNRTorch(AudioTransformPerChannel):
    ...
    def apply(self, y: torch.Tensor):
        snr = np.random.uniform(self.min_snr, self.max_snr)
        if y.ndim == 1:
            white_noise = torch.randn(len(y))  # for 1ch
        elif y.ndim == 2:
            white_noise = torch.randn(y.shape[0], y.shape[1])  # for 2ch
        #print(y.shape, white_noise.shape, snr)
